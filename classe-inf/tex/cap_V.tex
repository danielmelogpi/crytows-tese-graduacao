...
Legenda: representação de uma operação de criptografia com chaveiro em dispositivo móvel
...
\begin{table}[]
\centering
\caption{Atributos da mensagem JSON de pedido de criptografia de conteúdo}
\label{my-label}
\begin{tabular}{lllll}
...
\begin{itemize}
\item \textbf{cipherContent} (String) - Mensagem criptografada com a chave pública do usuário informado em ?receiver?, no ato da requisição de criptografia
\item \textbf{code} (Número inteiro) - Código HTTP referente ao sucesso ou falha da operação
...
%%   \caption{\texttt{Resposta da consulta à pedido de criptografia concluído} }
...
Exemplo em caso de processamento concluído:
{
    ...
    "code": 200,
    "statusMessage": ?Requisicão processada com sucesso?
}
...
Exemplo em caso de processamento ainda em andamento:
{
    "plainContent": "",
    "code": 404,
    "statusMessage": ?O processamento ainda não foi concluído.?
}
